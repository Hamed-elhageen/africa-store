...
import { Reset_ForgetPasswordResponse } from '../models/reset-password-response';
...
//this if the function that will send the email to the server , if everything is good at server .. it will return a code of four numbers , if there is an error , it will be handeled , all of this in the component that will use the service which is now update password
    sendVerifyCode(handle:string):Observable<Reset_ForgetPasswordResponse>{
        const url = environment.api + '/auth/password/forgot_password';
        return this.httpClient.post<Reset_ForgetPasswordResponse>(url, {handle}).pipe(
            catchError((err)=>{
    ...
    verifyResetCode(handle: string, code: string): Observable<VerificationResponse> {
        return this.httpClient.post<VerificationResponse>(environment.api + "/auth/password/validate_code", { handle, code }).pipe(
            catchError(err => {
...
resetPassword(handle: string, code:string, password:string, password_confirmation:string):Observable<Reset_ForgetPasswordResponse> {
    return this.httpClient.post<Reset_ForgetPasswordResponse>(
