...
import { VerifyRegisterResponse } from '../models/verify-register-response';
@Injectable({
  providedIn: 'root'
})
export class RegisterService {
  ...
  constructor(private httpClient:HttpClient,                                                                                                                               //this is our constructor where to inject the services like http client which is responsible for sending http requests and router which responsibel for navigation and platform id which is resposible for checking if the place where running the project in a browser to safely use local storage
    ...
    verifyUser( handle:string ,code:string) : Observable<VerifyRegisterResponse>{                                                                                                          //handle here is the email that will be sent with the request, and we send it with this name to be like the data the backend waiting for
        return  this.httpClient.post<VerifyRegisterResponse>(environment.api+"/auth/verify_user",{handle,code}).pipe(
            catchError(err=>{
