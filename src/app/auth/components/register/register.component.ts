        ...
        this.registerForm = new FormGroup(
            {
                nameInput: new FormControl('', [
                    Validators.required,
                    Validators.pattern(/^(?![0-9]+$)[a-zA-Z\u0600-\u06FF\s]+$/),
                ]),
    ...
    register() {
        if (this.registerForm.valid) {
            ...
            this.registerService
                ...
                .subscribe({
                    //here we used the function register which is in the register service and it takes from us the form data and if every thing is good at the server and the user is not logged before , it will return a token
                    next: (response) => {
                        this.ngxSpinner.hide();
                        this.router.navigateByUrl(
                            '/authentication/verifyregister'
                        ); //this token will be passed to the handleSuccess function which is declared under and will add the token to the local storage and update isUserLoggedSubject to true
                        localStorage.setItem(
                            'handle',
                            this.registerForm.value.emailInput
                        ); //now i save the email to the local storage because i will use it in the verify register to send with the code (not needing to put input field and take it from the user again => best user experience)
                        Toast.fire({
                            icon: 'success',
    ...
    handleError(error: any) {
        ...
        if (error instanceof HttpErrorResponse) {
            ...
            // باقي الأكواد
                ...
                this.errorMessage =
                    error.error?.message || 'البيانات غير صحيحة';
            } else if (error.status === 500) {
                this.errorMessage = error.error?.message || 'حدث خطأ في الخادم';
            } else if (error.status === 0) {
                this.errorMessage =
                    error.error?.message || 'تأكد من اتصال الإنترنت';
            } else {
